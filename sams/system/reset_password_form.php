...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $updateAccountStmt = $conn->prepare("UPDATE AccountManagement SET Password = ? WHERE UserID = ?");
    $updateAccountStmt->bind_param("ss", $newPassword, $userID);
    $updateAccountStmt->execute();

    echo "<script>alert('使用者:" . $userID . " 的密碼已成功更新'); window.location.href='reset_password.php';</script>";
...
<body>
            ...
            <div class="container">
                <div class="row align-items-center">
                    <div class="col-md-12 text-md-start text-center py-6">
                        <h1 class="fw-bold font-sans-serif mb-5">重設密碼 for User ID: <?php echo htmlspecialchars($userID); ?></h1>
  ...
  <link href="static/css/css2.css" rel="stylesheet">
    ...
    <scriptThe PHP code and HTML structure you provided set up a user interface for resetting a password, where administrators can enter a new password for a user after searching by their ID. The PHP script handles updating the password in the `AccountManagement` database table based on the user's ID, and uses prepared statements to secure the database operations against SQL injection. The layout includes a form within a styled webpage that aligns with your previous design templates. This approach ensures that the new password is securely updated in the database and provides feedback to the administrator through an alert that confirms the update's success.
