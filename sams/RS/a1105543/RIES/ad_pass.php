...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['fail'])) {//審核失敗
        require 'connect.php';

$info_id = $_GET['info_id'];

// 刪除照片
$sql = "DELETE FROM rentalimages WHERE InformationID = ?";
...
ob_end_flush();
  ...
  // 準備SQL語句
  $sql = "UPDATE rentalinformation SET Review = ? WHERE InformationID = ?";
  
  // 預處理和綁定
  $stmt = $conn->prepare($sql);
  
  $stmt->bind_param("ss", $valid,$info_id);
...
<body>
    ...
    <section class="pt-6 bg-600" id="home">
            <div class="container">
                ...
                <form method="post">
                    <input type="submit" name="fail" value="不合格">
                    <input type="submit" name="pass" value="通過">
                </form>
            </div>
        </section>
